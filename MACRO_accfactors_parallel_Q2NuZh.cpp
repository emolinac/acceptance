void MACRO_accfactors_parallel_Q2NuZh(Int_t N_Q2, Int_t N_Nu, Int_t N_Zh, Int_t N_Pt2, Int_t N_Phi, Int_t Q2_bin_selected, Int_t Nu_bin_selected, Int_t Zh_bin_selected, TString material, Int_t nSimuFiles, TString fsimul_loc, TString facc_target_loc, TString fbin_source_loc)
{
  /*FILES CUTS*/
  TCut Target_cut2;
  ...
  TFile* facc_target = new TFile(Form("./accfactors_"+material+"_%i%i%i_%i%i%i%i%i.root",Q2_bin_selected,Nu_bin_selected,Zh_bin_selected,N_Q2,N_Nu,N_Zh,N_Pt2,N_Phi),"RECREATE");
  ...
  std::cout<<"Q2 bin selected "<<Q2_bin_selected<<" Nu bin selected "<<Nu_bin_selected<<" Zh bin selected "<<Zh_bin_selected<<std::endl;
  ...
  TCut Zh_cut_S     = Form("Zh>%f && Zh<%f", Zh_min_event, Zh_max_event);
  TCut YC_cut_S     = "YC>0.1||YC<-0.1";
  TCut deltaZ_cut_S = "deltaZ<3&&deltaZ>-3";
  TCut Xf_cut_S     = "Xf>0";
  TCut cuts_simul   = Q2_cut_S&&Nu_cut_S&&Zh_cut_S&&deltaZ_cut_S;
  ...
  TCut Pt2_cut, Phi_cut, cuts_loop, cuts_Xf_loop;
  for(Int_t acc_entry = 0 ; acc_entry < limits_entries ; acc_entry++)
    {
      limits_tuple->GetEntry(acc_entry);
      if(Q2_bin==Q2_bin_selected&&Nu_bin==Nu_bin_selected&&Zh_bin==Zh_bin_selected)
