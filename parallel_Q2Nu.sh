...
then
	...
	do
	    echo $i $j
	    echo Sending jobs for "$N_Q2 $N_NU $N_ZH $N_PT2 $N_PHI $i $j $target $NSIMUFILES $simul_location $target_location $binning_location"
	    sbatch JOB_MACRO_accfactors_Q2Nuparallel.sh $N_Q2 $N_NU $N_ZH $N_PT2 $N_PHI $i $j $target $NSIMUFILES $simul_location $target_location $binning_location
	done
    done
